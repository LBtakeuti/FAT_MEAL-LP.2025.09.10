      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white shadow rounded-lg">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <h1 className="text-2xl font-bold text-gray-900">新規フィードバック作成</h1>
            <button
              type="button"
              ...
              <div className="mt-1 space-y-3">
                <div className="flex gap-2">
                  <button
                    ...
                    className={`px-3 py-1.5 text-sm font-medium rounded-md ${!showThumbnailUrlInput ? 'bg-orange-100 text-orange-700 border border-orange-300' : 'bg-gray-100 text-gray-700 border border-gray-300'}`}
                  >
                    ファイルアップロード
                  </button>
                  <button
                ...
                {!showThumbnailUrlInput ? (
                  <>
                    ...
                    />
                    {uploadingThumbnail && <p className="text-sm text-gray-500">アップロード中...</p>}
                  </>
                ) : (
                  <div className="space-y-2">
                    <input
                      ...
                      placeholder="https://drive.google.com/file/d/... または画像URL"
                      className="block w-full border-gray-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-sm"
                    />
                    <p className="text-xs text-gray-500">Google Driveの共有リンクを貼り付けると自動変換されます</p>
                    <button
                      type="button"
                      onClick={() => {
                        if (thumbnailUrlInput) {
                          setFormData({ ...formData, thumbnail_image: convertGoogleDriveUrl(thumbnailUrlInput) });
